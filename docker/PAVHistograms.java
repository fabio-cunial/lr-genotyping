...
/**
 * Prints histograms on the following PAV tags:
 *
 * HOM_REF    Perfect breakpoint homology (SV sequence vs reference). Format 
 *            'X,Y' where X homology upstream, and Y is homology downstream. 
 *            Homology vs reference is often better for DEL.
 * HOM_TIG    Perfect breakpoint homology (SV sequence vs contig). Format 'X,Y' 
 *            where X homology upstream, and Y is homology downstream. Homology
 *            vs contig is often better for INS.
 */
public class PAVHistograms {
    /**
     * @param args
     */
    public static void main(String[] args) throws IOException {
        ...
        while (str!=null) {
            ...
            if (str!=null) {
                ...
                histograms[i][0]++;
                i=Integer.parseInt(str.substring(j+1));
                if (i<0) i=-i;
                if (i>=histograms.length) i=histograms.length-1;
                histograms[i][0]++;
            }
            str=getField(tokens[7],"HOM_TIG");
            if (str!=null) {
                j=str.indexOf(",");
                i=Integer.parseInt(str.substring(0,j));
                if (i<0) i=-i;
