...
/**
 * Prints histograms on the following PBSV tags:
 *
 * CIPOS      Confidence interval around POS for imprecise variants.
 * SAC        Number of reads on the forward and reverse strand supporting each
 *            allele including reference.
 * IMPRECISE  (Flag) Imprecise structural variation.
 */
public class PBSVHistograms {
    /**
     * @param args
     */
    public static void main(String[] args) throws IOException {
        ...
        while (str!=null) {
            ...
            str=getField(tokens[7],"SAC");
            if (str!=null) {
                i=Integer.parseInt(str);
                if (i>=histograms.length) i=histograms.length-1;
                histograms[i][1]++;
            }
            histograms[tokens[7].indexOf("IMPRECISE")>=0?1:0][2]++;
            str=br.readLine();
        }
        br.close();
